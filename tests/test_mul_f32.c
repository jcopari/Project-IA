...
static int run_test_case(uint32_t N, int test_num) {
    ...
    mul_f32_ref(a_data, b_data, ref_data, N);
    
    // Execute AVX2 Mul
    err = q_mul_f32_avx2(&a_tensor, &b_tensor, &output_tensor);
    if (err != Q_OK) {
        printf("  ✗ FAILED: q_mul_f32_avx2 returned error: %s\n", q_strerror(err));
...
static int run_inplace_test(uint32_t N, int test_num) {
    ...
    mul_f32_ref(ref_data, b_data, ref_data, N);
    
    // Execute AVX2 Mul (in-place: output == a)
    err = q_mul_f32_avx2(&a_tensor, &b_tensor, &a_tensor);
    if (err != Q_OK) {
        printf("  ✗ FAILED: q_mul_f32_avx2 returned error: %s\n", q_strerror(err));
...
int main(void) {
    srand(time(NULL));
    printf("========================================\n");
    printf("Element-wise Mul FP32 AVX2 Test Suite\n");
    printf("========================================\n");
    ...
    // NULL a
    q_error_code err = q_mul_f32_avx2(NULL, &b_err, &output_err);
    assert(err == Q_ERR_INVALID_ARG);
    printf("  ✓ PASSED: NULL a validation\n");

    // NULL b
    err = q_mul_f32_avx2(&a_err, NULL, &output_err);
    assert(err == Q_ERR_INVALID_ARG);
    printf("  ✓ PASSED: NULL b validation\n");

    // NULL output
    err = q_mul_f32_avx2(&a_err, &b_err, NULL);
    assert(err == Q_ERR_INVALID_ARG);
    ...
    err = q_mul_f32_avx2(&a_err, &shape_mismatch, &output_err);
    ...
    if (total_failures == 0) {
        printf("\n✓ All Element-wise Mul FP32 tests PASSED\n");
        return 0;
    } else {
        printf("\n✗ %d Element-wise Mul FP32 test case(s) FAILED\n", total_failures);
        return 1;
    }
}
