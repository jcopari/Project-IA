# Tensor Add AVX2: Mathematical Proof
...
- **Implementation:** Do NOT use `restrict` on `output` parameter to allow aliasing.
...
- **Mathematical Proof:** 
  - For each element `i`:
    ...
    - Compute `a[i] + b[i]` using `_mm256_add_ps`
...
### NaN/Inf Propagation
- **IEEE 754:** Guarantees correct propagation: `NaN + x = NaN`, `Inf + x = Inf` (unless x is -Inf).
...
## NUMERICAL STABILITY
- **Element-wise Addition:** No accumulation of errors. Each operation is independent.
- **No Rounding Errors:** Direct addition, no intermediate accumulations.
